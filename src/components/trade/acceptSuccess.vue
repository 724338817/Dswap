<template>
  <el-dialog width="500px" :visible.sync="dialogVisible" :before-close="Close" :close-on-click-modal="false">
    <span class="title">Your accept is complete！</span>
    <div class="nft_data">
      <img :src="acceptSuccessData.imageUrl" />
      <div class="nft_info">
        <span>Your accepted  <span class="info_color">{{acceptSuccessData.name}}</span> from the
        <span class="info_color">{{ acceptSuccessData.collectionName }}</span> collection</span>
      </div>
    </div>
    <div class="line"></div>
    <div class="tip">
      <div class="pay_title">
        ...
        <span class="addr">{{ acceptSuccessData.transactionHash }}</span>
      </div>
    </div>
    <span slot="footer" class="dialog-footer">
      <!-- <el-button class="list" type="" @click="submit('sellValidateForm')">List for sale</el-button> -->
      <el-button class="makeOffer" type="primary" @click="submit('sellValidateForm')">View item</el-button>
    </span>
  </el-dialog>
</template>
