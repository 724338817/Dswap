<template>
  <el-dialog width="500px" :visible.sync="dialogVisible" :before-close="Close" :close-on-click-modal="false">
    <span class="title">Approve MakeOffer</span>
    <div class="nft_data">
      <img :src="makeOfferApproveData.imageUrl" />
      <div class="nft_info">
        <p class="bold_Font">{{makeOfferApproveData.name}}</p>
        <p>{{ makeOfferApproveData.collectionName }}  #{{ makeOfferApproveData.tokenId }}</p>
        <span>Owned by </span>
        <!-- <span>{{makeOfferApproveData.contractDeployer.substring(0,20)}}</span> -->
        <span>{{makeOfferApproveData.owners}}</span>
        <p class="bold_Font">{{makeOfferApproveData.price}} {{ unit }}</p>
      </div>
    </div>
    <div class="line"></div>
    <div class="tip">
      ...
      <div class="pay_value">
        <span>You'll be asked to approve this MakeOffer from your wallet.</span>
      </div>
    </div>
  </el-dialog>
</template>
