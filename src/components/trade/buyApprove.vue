<template>
  <el-dialog width="500px" :visible.sync="dialogVisible" :before-close="Close" :close-on-click-modal="false">
    <span class="title">Approve purchase</span>
    <div class="nft_data">
      <img :src="buyApproveData.imageUrl" />
      <div class="nft_info">
        <p class="bold_Font">{{buyApproveData.name}}</p>
        <!-- <p>{{ buyApproveData.collectionName }}</p> -->
        <p>{{ buyApproveData.collectionName }}  #{{ buyApproveData.tokenId }}</p>

        <span>Owned by </span>
        <!-- <span>{{buyApproveData.contractDeployer.substring(0,20)}}</span> -->
        <span>{{buyApproveData.owners}}</span>
        <p class="bold_Font">{{buyApproveData.price}} {{ unit }}</p>
      </div>
    </div>
    <div class="line"></div>
    <div class="tip">
