<template>
  <el-dialog width="500px" :visible.sync="dialogVisible" :before-close="Close" :close-on-click-modal="false">
    <span class="title">Your accept is processing…</span>
    <div class="nft_data">
      <img class="" src="../../assets/image/icon_load.png" />
      <div class="nft_info">
        <span>Your accept of <span class="info_color">{{acceptProcessData.name}}</span> is processing. It should be confirmed on the blockchain shortly.</span>
        <!-- <span>{{sellPriceData.contractDeployer.substring(0,20)}}</span> -->
      </div>
    </div>
    <div class="line"></div>
    <div class="tip">
      <!-- <div class="pay_title">
        ...
        <span class="addr">{{buyProcessData.id}}</span>
      </div> -->
    </div>
  </el-dialog>
</template>
